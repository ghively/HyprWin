...
impl HotkeyManager {
    ...
    pub fn register(&mut self, hotkey: Hotkey) -> anyhow::Result<u32> {
        ...
        if vk.is_none() {
            anyhow::bail!("Unknown key name: {}", hotkey.key);
        }
        let vk = vk.unwrap();
    ...
    pub fn handle_message(&self, wparam: WPARAM, _lparam: LPARAM) -> Option<&Hotkey> {
        ...
        if hk.is_some() {
            trace!("Hotkey triggered: id={} action={}", id, hk.as_ref().unwrap().action);
        }
        hk
    }
    ...
    unsafe {
        ...
        loop {
            ...
            if msg.message == WM_HOTKEY {
                ...
                // The hotkey manager will map this to an action
                // For now we send the raw ID and let the caller handle mapping
                // We send the action string directly if we have the manager reference
            }
