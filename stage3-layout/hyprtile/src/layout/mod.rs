...
pub fn calculate_layout(
    ...
    match layout {
        ...
        LayoutType::MasterStack => {
            // Use default master-stack configuration.
            let config = master_stack::MasterStackConfig::default();
...
mod tests {
    ...
    fn test_calculate_layout_dwindle() {
        ...
        let gaps = test_gaps();
        let result = calculate_layout(LayoutType::Dwindle, &windows, &workspace, &gaps, 0);
    ...
    fn test_calculate_layout_master_stack() {
        ...
        let gaps = test_gaps();
        let result = calculate_layout(LayoutType::MasterStack, &windows, &workspace, &gaps, 0);
    ...
    fn test_calculate_layout_monocle() {
        ...
        let windows = vec![wid(1), wid(2), wid(3)];
        let gaps = test_gaps();
        let result = calculate_layout(LayoutType::Monocle, &windows, &workspace, &gaps, 1);
        assert_eq!(result.len(), 3);
    ...
    fn test_calculate_layout_grid() {
        ...
        let gaps = test_gaps();
        let result = calculate_layout(LayoutType::Grid, &windows, &workspace, &gaps, 0);
    ...
    fn test_calculate_layout_empty() {
        ...
        for layout in LayoutType::all() {
            let result = calculate_layout(layout, &[], &workspace, &gaps, 0);
            assert!(
                result.is_empty(),
    ...
    fn test_layout_engine_calculate() {
        ...
        let windows = vec![wid(1), wid(2)];
        let gaps = test_gaps();
        let result = engine.calculate(&windows, &workspace, &gaps);
        assert_eq!(result.len(), 2);
        for (_, rect) in &result {
            assert!(rect.width > 0);
            assert!(rect.height > 0);
    ...
    fn test_layout_engine_calculate_empty() {
        ...
        let workspace = Rect::new(0, 0, 1000, 600);
        let gaps = test_gaps();
        let result = engine.calculate(&[], &workspace, &gaps);
